    ...
    <div className={`min-h-screen transition-all duration-300 ${config.isDiscreetMode ? 'bg-zinc-100 text-zinc-900 grayscale' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 bg-slate-900 p-8 rounded-3xl shadow-2xl text-white relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-2">Integrated Global Alert & Follow-up</h2>
              ...
              <div className="flex flex-wrap gap-4 items-center">
                <button onClick={() => setIsAlertModalOpen(true)} className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-wider hover:bg-blue-700 transition-all flex items-center gap-2 shadow-xl shadow-blue-500/20">
                  Launch Global Alert
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                </button>
                <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white/5 px-4 py-3 rounded-xl border border-white/10">
                  Daily Demand: <span className="text-red-400">8-12 Major Surgeries</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-16 h-16 bg-red-50 group-hover:bg-red-100 rounded-bl-3xl -mr-4 -mt-4 transition-colors"></div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Stock Readiness</p>
              <h3 className="text-3xl font-black text-slate-900">{currentStockInfo.available} Units</h3>
              ...
              <p className="text-[10px] text-red-500 mt-2 font-bold uppercase">{currentStockInfo.percentage}% of Daily Demand</p>
            </div>
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Human Connectivity</p>
              <h3 className="text-3xl font-black text-slate-900">{MOCK_SURGERY_DATA[0].bloodUnitsRequired}+ Potential</h3>
              <p className="text-[10px] text-blue-500 mt-2 font-bold uppercase tracking-tighter">Follow-up Registry Online</p>
            </div>
          </div>
        </div>
